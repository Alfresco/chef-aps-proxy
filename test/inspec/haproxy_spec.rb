control 'chef-aps-02' do
  ...
  describe file('/etc/haproxy/haproxy.cfg') do
    it { should exist }
    it { should be_file }
    its('content') { should match 'server localhost localhost:8080 check inter 5000' }
    ...
    its('content') { should match 'use_backend activiti' }
    its('content') { should match 'option httpchk activiti-app/app/authentication/' }
    its('content') { should match 'pidfile /var/run/haproxy.pid' }
    its('content') { should match 'uid 801' }
  end
end
